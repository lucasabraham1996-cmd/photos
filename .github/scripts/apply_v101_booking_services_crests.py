...
insert=needle+"""const NIGHT_MATCH_PRICE=75000;
...
old="async function createDraft(data){const id=uid();const clean={id,type:'booking',status:'pendiente',formCompleted:false,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),payment:Number(data.payment||0),adminNote:data.adminNote||'',date:'',home:'',away:'',category:'',venue:'',address:'',matchTime:'',arrivalTime:'',clientName:'',phone:'',notes:''};await bookingDoc(id).set(clean);return clean}"
new="async function createDraft(data){const id=uid();const service=data.service==='night'?'night':'custom';const payment=service==='night'?NIGHT_MATCH_PRICE:Number(data.payment||0);const clean={id,type:'booking',status:'pendiente',formCompleted:false,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),service,payment,adminNote:data.adminNote||'',date:'',home:'',away:'',category:'',venue:'',address:'',matchTime:'',endTime:'',arrivalTime:'',clientName:'',phone:'',notes:''};await bookingDoc(id).set(clean);return clean}"
replace_once(old,new,'createDraft')

old="function isAgendaBooking(b){if(!b)return false;if(b.formCompleted===true||b.confirmedAt)return true;return ['confirmado','realizado','cancelado'].includes(String(b.status||'').toLowerCase())&&Boolean(b.date&&b.home&&b.away)}"
new="function isAgendaBooking(b){if(!b)return false;const completed=Boolean(b.formCompleted===true||b.confirmedAt);const core=Boolean(b.date&&b.matchTime&&b.home&&b.away&&b.category&&b.venue&&b.address&&b.arrivalTime&&b.clientName&&b.phone);const hasEndTime=!Object.prototype.hasOwnProperty.call(b,'endTime')||Boolean(b.endTime);return completed&&core&&hasEndTime}"
replace_once(old,new,'isAgendaBooking')

# ---------- Public form: final time, service, live night price ----------
old='<div class=\\"field\\"><label>Horario del partido</label><input required class=\\"input\\" type=\\"time\\" name=\\"matchTime\\" value=\\"${esc(b.matchTime)}\\"></div>${teamPickerMarkup(\'home\',\'Equipo local\',b.home)}'
new='<div class=\\"field\\"><label>Horario del partido</label><input required class=\\"input\\" type=\\"time\\" name=\\"matchTime\\" value=\\"${esc(b.matchTime)}\\"></div><div class=\\"field\\"><label>Hora de finalización</label><input required class=\\"input\\" type=\\"time\\" name=\\"endTime\\" value=\\"${esc(b.endTime||\'\')}\\"></div>${teamPickerMarkup(\'home\',\'Equipo local\',b.home)}'
replace_once(old,new,'public end time')

old='<div class=\\"field\\"><label>Pago acordado</label><input class=\\"input\\" readonly value=\\"${money(b.payment)}\\"></div><div class=\\"field\\"><label>Tu nombre</label>'
new='<div class=\\"field\\"><label>Servicio</label><input id=\\"publicServiceDisplay\\" class=\\"input\\" readonly value=\\"${esc(bookingServiceName(b))}\\"></div><div class=\\"field\\"><label>Pago acordado</label><input id=\\"publicPaymentDisplay\\" class=\\"input\\" readonly value=\\"${money(b.payment)}\\"></div><div class=\\"field\\"><label>Tu nombre</label>'
replace_once(old,new,'public service display')

old="function publicForm(b){currentDraft=b;document.title=`${b.id} · Contratación`;"
helper="""function wirePublicNightPricing(b){const form=$('#clientForm');if(!form)return;const end=form.querySelector('[name=\"endTime\"]'),pay=$('#publicPaymentDisplay'),service=$('#publicServiceDisplay');const refresh=()=>{const pr=bookingPriceFor(end&&end.value,b.payment,b.service);if(pay)pay.value=money(pr.payment);if(service)service.value=pr.night?'Partido nocturno':'Cobertura de partido'};if(end){end.addEventListener('input',refresh);end.addEventListener('change',refresh)}refresh()}
function publicForm(b){currentDraft=b;document.title=`${b.id} · Contratación`;"""
replace_once(old,helper,'public pricing helper')

old=";initTeamPickers();$('#clientForm').onsubmit=submitClient}"
new=";initTeamPickers();wirePublicNightPricing(b);$('#clientForm').onsubmit=submitClient}"
replace_once(old,new,'wire public pricing')
...
# ---------- Admin card: show final time and service ----------
old='<span>⚽ Partido <strong>${esc(b.matchTime||\'--:--\')}</strong></span><span>💰 <strong>${money(b.payment)}</strong></span>'
new='<span>⚽ Partido <strong>${esc(b.matchTime||\'--:--\')}</strong></span><span>🏁 Finaliza <strong>${esc(b.endTime||\'--:--\')}</strong></span><span>📋 <strong>${esc(bookingServiceName(b))}</strong></span><span>💰 <strong>${money(b.payment)}</strong></span>'
replace_once(old,new,'booking card service')

# ---------- Edit modal: visual team pickers + end time + service ----------
old='<div class=\\"field\\"><label>Equipo local</label><input class=\\"input\\" name=\\"home\\" value=\\"${esc(titleWords(b.home))}\\"></div><div class=\\"field\\"><label>Equipo visitante</label><input class=\\"input\\" name=\\"away\\" value=\\"${esc(titleWords(b.away))}\\"></div>'
...
old='<div class=\\"field\\"><label>Hora partido</label><input class=\\"input\\" type=\\"time\\" name=\\"matchTime\\" value=\\"${esc(b.matchTime||\'\')}\\"></div>'
new='<div class=\\"field\\"><label>Hora partido</label><input class=\\"input\\" type=\\"time\\" name=\\"matchTime\\" value=\\"${esc(b.matchTime||\'\')}\\"></div><div class=\\"field\\"><label>Hora de finalización</label><input class=\\"input\\" type=\\"time\\" name=\\"endTime\\" value=\\"${esc(b.endTime||\'\')}\\"></div>'
replace_once(old,new,'edit end time')

old='<div class=\\"field\\"><label>Pago acordado</label><input class=\\"input\\" type=\\"number\\" min=\\"0\\" name=\\"payment\\" value=\\"${esc(b.payment||0)}\\"></div><div class=\\"field\\"><label>Estado</label>'
new='<div class=\\"field\\"><label>Servicio</label><select class=\\"input\\" name=\\"service\\" id=\\"editService\\"><option value=\\"custom\\" ${bookingIsNight(b)?\'\':\'selected\'}>Cobertura de partido</option><option value=\\"night\\" ${bookingIsNight(b)?\'selected\':\'\'}>Partido nocturno · $75.000</option></select></div><div class=\\"field\\"><label>Pago acordado</label><input class=\\"input\\" type=\\"number\\" min=\\"0\\" name=\\"payment\\" value=\\"${esc(b.payment||0)}\\"></div><div class=\\"field\\"><label>Estado</label>'
replace_once(old,new,'edit service')

old=";document.body.appendChild(modal);const close=()=>modal.remove();$('#closeEdit').onclick=close;"
new=";document.body.appendChild(modal);initTeamPickers();const editForm=$('#editBookingForm'),editEnd=editForm.querySelector('[name=\\"endTime\\"]'),editService=editForm.querySelector('[name=\\"service\\"]'),editPayment=editForm.querySelector('[name=\\"payment\\"]');const syncEditPricing=()=>{if(isNightMatchEndTime(editEnd.value)||editService.value==='night'){editService.value='night';editPayment.value=String(NIGHT_MATCH_PRICE)}};editEnd.addEventListener('change',syncEditPricing);editEnd.addEventListener('input',syncEditPricing);editService.addEventListener('change',syncEditPricing);syncEditPricing();const close=()=>modal.remove();$('#closeEdit').onclick=close;"
replace_once(old,new,'init edit pickers')

old="data.home=titleWords(data.home);data.away=titleWords(data.away);data.payment=Number(data.payment||0);$('#editMsg')"
new="data.home=titleWords(data.home);data.away=titleWords(data.away);const pricing=bookingPriceFor(data.endTime,data.payment,data.service);data.service=pricing.service;data.payment=pricing.payment;$('#editMsg')"
replace_once(old,new,'edit save pricing')

# ---------- New booking: service quick tariff ----------
old='<form id=\\"draftForm\\" class=\\"formgrid\\" style=\\"margin-top:17px\\"><div class=\\"field full\\"><label>Pago acordado</label><input required class=\\"input\\" type=\\"number\\" min=\\"0\\" name=\\"payment\\" placeholder=\\"Ej: 60000\\"></div>'
new='<form id=\\"draftForm\\" class=\\"formgrid\\" style=\\"margin-top:17px\\"><div class=\\"field full\\"><label>Servicio / tarifa</label><select class=\\"input\\" name=\\"service\\" id=\\"draftService\\"><option value=\\"custom\\">Tarifa personalizada</option><option value=\\"night\\">Partido nocturno · $75.000</option></select><div class=\\"muted\\">Partido nocturno: aplica cuando el partido finaliza a las 20:00 hs o después.</div></div><div class=\\"field full\\"><label>Pago acordado</label><input required class=\\"input\\" type=\\"number\\" min=\\"0\\" name=\\"payment\\" placeholder=\\"Ej: 60000\\"></div>'
replace_once(old,new,'new service selector')

old=";document.body.appendChild(modal);$('#closeModal').onclick=()=>modal.remove();$('#draftForm').onsubmit=async e=>"
new=";document.body.appendChild(modal);const draftService=$('#draftService'),draftPayment=$('#draftForm [name=\\"payment\\"]');draftService.onchange=()=>{if(draftService.value==='night')draftPayment.value=String(NIGHT_MATCH_PRICE)};$('#closeModal').onclick=()=>modal.remove();$('#draftForm').onsubmit=async e=>"
replace_once(old,new,'wire draft service')

# ---------- Calendar/details use final time ----------
old="function endDate(b){const s=startDate(b);if(!s)return null;return new Date(s.getTime()+3*60*60*1000)}"
new="function endDate(b){if(b&&b.date&&b.endTime){const e=new Date(`${b.date}T${b.endTime}:00`);if(!Number.isNaN(e.getTime()))return e}const s=startDate(b);if(!s)return null;return new Date(s.getTime()+3*60*60*1000)}"
replace_once(old,new,'calendar end time')

old="Hora del partido: ${b.matchTime||''}\\nLlegada fotógrafo: ${b.arrivalTime||''}\\nCliente:"
new="Hora del partido: ${b.matchTime||''}\\nFinalización: ${b.endTime||''}\\nServicio: ${bookingServiceName(b)}\\nLlegada fotógrafo: ${b.arrivalTime||''}\\nCliente:"
replace_once(old,new,'calendar details')
...
def clean_background(raw,out_path):
    ...
    w,h=im.size
    px=im.load()
    # Estimate background from corners + edge midpoints. Works for white and other flat backgrounds.
    ...
    bbox=alpha.getbbox()
    if not bbox:
        raise RuntimeError(f'background cleaning erased entire image: {out_path}')
    im=im.crop(bbox)
    # Normalize to a transparent 512 square while preserving aspect ratio.
    im.thumbnail((460,460),Image.Resampling.LANCZOS)
    canvas=Image.new('RGBA',(512,512),(0,0,0,0))
    x=(512-im.width)//2;y=(512-im.height)//2
...
for filename,url in sources.items():
    ...
    if len(r.content)<1500:
        raise RuntimeError(f'{filename}: response too small ({len(r.content)} bytes)')
